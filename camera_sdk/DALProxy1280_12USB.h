...
/** \defgroup Module_Management IRLugX1M3 Management
  * \brief Etablish and manage communication with IRLugX1M3.
  */

/** \defgroup Module_Processing IRLugX1M3 Processing
  * \brief Control IRLugX1M3 image processing.
  * Query module connected to workstation, Open and close link.
  */

/** \defgroup Module_Control IRLugX1M3 Control
...
/** \defgroup Module_Image IRLugX1M3 Image
  * \brief Query Image from IRLugX1M3.
  */

/** \defgroup Module_Storage IRLugX1M3 Storage
  * \brief Store and retrieve processing settings into IRLugX1M3.
  */

/** \defgroup Module_Calibration IRLugX1M3 Calibration
  * \brief IRLugX1M3 NUC, bad pixel and Shutterless correction calibration. Refer to calibration example provided with the SDK for detailed usage of the following function.
  *
...
DALProxy1280_12USB_API typedef enum
{
    ...
    eProxy1280_12USBHandleError, /**< Function call with wrong or invalid IRLugX1M3 handle. */
    eProxy1280_12USBInitFailed,  /**< Internal error occur. */
    eProxy1280_12USBOpenFailed,  /**< Open connection to IRLugX1M3 failed. Maybe already connected */
    eProxy1280_12USBCommFailed,  /**< Exchange with IRLugX1M3 failed. */
    eProxy1280_12USBTimeout, /**< Operation on IRLugX1M3 timeout before completed. */
    eProxy1280_12USBSyncBroken, /**< GetImage(), Sync with IRLugX1M3 broken. */
    //
    ...
    eProxy1280_12USBBistFailure, /**< IRLugX1M3 reported a Built-In Self Test error. */
    eProxy1280_12USBFormatFailed,  /**< Incompatible file format for IRLugX1M3. */
    eProxy1280_12USBFormatMismatch, /**< Incompatible calibration format for IRLugX1M3. */
    //
 ...
 *
 * This set provides :
 * \li Functions to enumerates and name plugged IRLugX1M3.
 * \li Function to connect and disconnect to IRLugX1M3.
 *
 * Application call Proxy1280_12USB_GetModuleCount() to know how many IRLugX1M3 are plugged to workstation.
 * First IRLugX1M3 index is 0, and so on.
 *
 * Calling Proxy1280_12USB_GetModuleCount() check IRLugX1M3 count. So, call it will refresh IRLugX1M3 list.
 *
 * Before calling any other function's group, Application must connect to a IRLugX1M3 using Proxy1280_12USB_ConnectToModule().<br>
 * Once a IRLugX1M3 is connected by an application, it's not available to another application.
 * Application must release IRLugX1M3 by calling Proxy1280_12USB_DisconnectFromModule().
 *
 * Connection to IRLugX1M3 will provide a \em handle. This \em handle is use by all functions addressing this IRLugX1M3.
 * It remains valid until Proxy1280_12USB_DisconnectFromModule() is called.
 *
 * Application can connect several IRLugX1M3, using different \em handles.
* @{ */
...
/** Query IRLugX1M3 name by index.
 * \param[in] iIdx Module index.
 * \param[out] paName IRLugX1M3 name from index.
...
/** Connect to IRLugX1M3 by index.<br>
 * This function will return a handle, which will be uses as IRLugX1M3 identifier.<br>
 * Connection may failed if IRLugX1M3 is already connected by another application.
 * \param[in] iIdx Module index. First IRLugX1M3 index is 0.
 * \param[out] paHandle IRLugX1M3 handle.
 **/
 ...
 * This function will check if handle is still valid, and then check connection with IRLugX1M3.
 * \param[in] paHandle IRLugX1M3 handle.
...
/** Disconnect to IRLugX1M3 by index.
 * This function will release IRLugX1M3 connection.
 * \param[in] paHandle IRLugX1M3 handle.
 **/
DALProxy1280_12USB_API eDALProxy1280_12USBErr Proxy1280_12USB_DisconnectFromModule(HANDLE paHandle);

/** Run the IRLugX1M3 Built-In Self-Test.
 * This function will check if handle is still valid, and then run the built-in self tests.
 * \param[in] paHandle IRLugX1M3 handle.
 * \param[out] diagCode Diagnostic code provided by the IRLugX1M3. Value is 0 in case of success.
 * \return \ref eProxy1280_12USBSuccess on success, or error code.
 **/
 ...
 *
 * This set provides a single function to query current IRLugX1M3 image.
 ...
 *  IRLugX1M3 provides \b 8 slots to store Gain or Offset value. Slot are not dedicated to a kind of data.
 *  \attention Storage space is limited into IRLugX1M3. Hence, data (Gain or Offset) are rounded to fit into slot.
 ...
 * When application connect to IRLugX1M3, Proxy1280_12USB_ConnectToModule() function will look for default slot,
 ...
 * \note No need to call and use this function (already done at IRLugX1M3 connection)
 **/
...
/** Retrieve IRLugX1M3 slot data as Offset values.<br>
 * This function may failed if slot is empty, or slot data are not Offset values.
 * \param[in] paHandle IRLugX1M3 handle.
 * \param[in] paiIndex Slot index as data.
 * \param[out] paTableOffset Offset values from IRLugX1M3 slot.
 * \param[out] paData Table associate data. NULL, or 60 bytes placeholder. paData is additional data associated to Gain or Offset array, which can be used freely by application for instance to keep a trace of Gain or Offset table calibration conditions, either sensitivity, either focal plane array temperature.
 */
...
/** Retrieve bad pixel position in image from IRLugX1M3.
 * \param[in] paHandle IRLugX1M3 handle.
...
/** Save Gain values into IRLugX1M3 slot data.
 * \param[in] paHandle IRLugX1M3 handle.
 * \param[in] paiIndex Slot index.
 * \param[in] paTableGain Gain values to store into IRLugX1M3 slot.
...
/** Save Offset values into IRLugX1M3 slot data.
 * \param[in] paHandle IRLugX1M3 handle.
 * \param[in] paiIndex Slot index.
 * \param[in] paTableOffset Offset values to store into IRLugX1M3 slot.
 * \param[in] paData Table associate data. NULL, or 60 bytes placeholder. paData is additional data associated to Gain or Offset array, which can be used freely by application for instance to keep a trace of Gain or Offset table calibration conditions, either sensitivity, either focal plane array temperature.
 */
...
/** Save bad pixel position into IRLugX1M3 slot data.
 * \param[in] paHandle IRLugX1M3 handle.
 * \param[in] paTableX,paTableY Bad pixels position in image.
 * \param[in] paCount bad pixels count.
*/
...
/** Use IRLugX1M3 slot data as Gain values for NUC processing,
 * i.e. retrieve it from IRLugX1M3, and set it to NUC processing.
 * \param[in] paHandle IRLugX1M3 handle.
 * \param[in] paiIndex Slot index as data.
 */
...
/** Use IRLugX1M3 slot data as Offset values for NUC processing,
 * i.e. retrieve it from IRLugX1M3, and set it to NUC processing.
 * \param[in] paHandle IRLugX1M3 handle.
 * \param[in] paiIndex Slot index as data.
 */
...
/** Use IRLugX1M3 stored bad pixel for bad pixel correction.
 * \param[in] paHandle IRLugX1M3 handle.
 */
DALProxy1280_12USB_API eDALProxy1280_12USBErr Proxy1280_12USB_LoadCurrentBadPixels(HANDLE paHandle);

/** Save current Gain values into IRLugX1M3 slot data.
 * \param[in] paHandle IRLugX1M3 handle.
 * \param[in] paiIndex Slot index.
 * \param[in] paData Table associate data. NULL, or 60 bytes placeholder. paData is additional data associated to Gain or Offset array, which can be used freely by application for instance to keep a trace of Gain or Offset table calibration conditions, either sensitivity, either focal plane array temperature.
 */
...
/** Save Shutterless Tables into IRLugX1M3.
 * \param[in] paHandle IRLugX1M3 handle.
...
/** Load Shutterless Tables from IRLugX1M3 in order to use it with shutterless processing
 * \param[in] paHandle IRLugX1M3 handle.
 * \return eProxy1280_12USBFeatureNotAvailable error if Shutterless is unavailable on the module.
*/
DALProxy1280_12USB_API eDALProxy1280_12USBErr Proxy1280_12USB_LoadCurrentShutterlessTables(HANDLE paHandle);

/** Clear Saved Calibration data format from IRLugX1M3 in order to be able to save the other format (to avoid format mismatch error).
 * \param[in] paHandle IRLugX1M3 handle.
 ...
 *
 * Must be use on correct sequencing with IRLugX1M3 shutterless module.
 *
 * \param[in] paHandle IRLugX1M3 handle.
 ...
 *
 * Must be used on correct sequencing with IRLugX1M3 shutterless module.
 *
 * \param[in] paHandle IRLugX1M3 handle.
