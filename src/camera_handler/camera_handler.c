...
CAM_HANDLER_API void paimage_address(int *p) { p = (int *)&paImage; }
...
CAM_HANDLER_API bool is_buffer_ready(void) { return buffer_filled; }

CAM_HANDLER_API HANDLE init_camera(float fps, bool SL, char BP, uint8_t agc,
                                   char nuc) {
    ...
    eDALProxy1280_12USBErr connect_result =
        Proxy1280_12USB_ConnectToModule(0, &camera_handle);
    ...
    for (int i = 0; i < NUM_TEST_FRAMES; i++) {
        res = Proxy1280_12USB_GetImage(camera_handle, paImage, paMeta,
                                       GETIMAGE_TIMEOUT);
        if (res != 0) {
            return NULL;
        }
...
CAM_HANDLER_API matrix_t *get_matrix_buffer(void) { return &frame_matrix; }
...
CAM_HANDLER_API int load_frame_buffer(HANDLE camera_handle) {
    ...
    err = Proxy1280_12USB_GetImage(camera_handle, paImage, paMeta,
                                   GETIMAGE_TIMEOUT);
...
CAM_HANDLER_API int sl_calibration_t0(HANDLE h, int iStage) {
    ...
    res = Proxy1280_12USB_GetNUCProcessing(h, &is_badpixels_enabled,
                                           &is_nuc_enabled);
    if (res != eProxy1280_12USBSuccess) {
        return res;
    }
    ...
    if (is_nuc_enabled) {
        ...
        res = Proxy1280_12USB_SetNUCProcessing(h, is_badpixels_enabled,
                                               is_nuc_enabled);
    }
